...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			var addFun = function(id,articleid, num, type,storage){
				// 购买的数量
				...
				switch (type){
					case 1: // 增加一个
						window.location.href = "updateCar.action?id=" + id + "&buynum=" + (buyNum+1) + "&articleid=" +articleid;
						break;
					case 2: // 减少一个
						window.location.href = "updateCar.action?id=" + id + "&buynum=" + (buyNum-1) + "&articleid=" +articleid;
						break;
			...
			function submitOrder(){
				var boxs = document.getElementsByName("box"); // 购物车id
				//保存传递到后台的数据
				var orderInfo = document.getElementById("orderInfo");
				
				var num = 0;
				//在Js中foreach并不会遍历每个元素,而是遍历Key,即所有属性
				for (var i = 0; i < boxs.length; i++) {//把购物车打勾后结算的商品进行遍历，如果购物车商品id不同则用，分开
					//全选事件点击，其他复选框和全选框状态相同
					var box = boxs[i];
					if(box.checked){
						num++;
						if(orderInfo.value == null ||  orderInfo.value == ""){
							...
							//判断是否已经存在
							orderInfo.value = orderInfo.value + ","+box.value;  // 1，2
						}
					}
				}
	        ...
	        <!-- 购物车不为空 <input  type="checkbox"  name="${shopCar.article.id}">-->
							...
							<div class="shoppingcart_promotions_01">
								<ul class="shoppingcart_list">
									...
									<li class="row55">
										...
										<a href="javascript:void(0);" onclick="addFun(${shopCar.id},${shopCar.article.id},${shopCar.buynum},1,${shopCar.article.storage});" title="数量加一" class="add">+</a>
										<a href="javascript:void(0);" onclick="addFun(${shopCar.id},${shopCar.article.id},${shopCar.buynum},2,${shopCar.article.storage});" title="数量减一" class="cut">-</a>
									</li>
									<li class="row66">
										<a name="movetofavorite" href="javascript:void(0);" 
										title="此商品将移至“我的收藏”并从购物车中删除" id="move_20376113">移入收藏</a>
										<a name="delete" href="javascript:void(0);" onclick="deleteFn(${shopCar.id});">删除</a>
									</li>
								</ul>
							</div>
						</c:forEach>
		        ...
		        <div align="right" id="div_total" style="display: block;clear:both;">
		            ...
		            <p>
		                ...
		                <form action="${ctx}/order.action" method="post" id="form">
